...
prc_avg = train_df.select(avg("price")).first()[0]
prc_med = train_df.select(median("price")).first()[0]

pred_df = (test_df.withColumn("avgPrediction", lit(prc_avg))
                  .withColumn("medianPrediction", lit(prc_med)))
